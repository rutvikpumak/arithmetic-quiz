...
export function QuizCard({ quizNo }) {
  ...
  const nextHandler = () => {
    if (state[`quiz${quizNo}`]?.currentQue !== +state[`quiz${quizNo}`]?.noOfQuestion) {
  ...
  useEffect(() => {
    if (state[`quiz${quizNo}`]?.currentQue !== +state[`quiz${quizNo}`]?.noOfQuestion - 1) {
    ...
    <div className="flex flex-col m-4 py-2 px-4 gap-4">
      <>
        <p className="text-center font-bold text-xl underline underline-offset-4">
          Question {state[`quiz${quizNo}`]?.currentQue + 1}/
          {state[`quiz${quizNo}`]?.questions.length}
        </p>
